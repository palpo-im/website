...
- [compose.with-caddy.yml](https://github.com/palpo-im/palpo/blob/main/deploy/docker/compose.with-caddy.yml)
    ...
    如果您想要一个开箱即用的 `caddy-docker-proxy` 设置，请使用此配置，此配置添加了 [Caddy](https://caddyserver.com/) 作为反向代理服务器。使用时需将所有 `example.com` 占位符替换为您自己的域名。
    你还需要在启动前创建 `caddy` 网络：
...
<details>
<summary>故障排除：GHCR 认证错误</summary>

如果您遇到 `Error response from daemon: Get "https://registry-1.docker.io/v2/": unauthorized: incorrect username or password` 错误，请按照以下步骤解决：

**步骤 1**：生成 GitHub Token — 访问 GitHub → Settings → Developer settings → Personal access tokens → Tokens (classic)。勾选 `read:packages` 权限。
...
| Docker Hub | [docker.io/palpo/palpo:latest][dh] | ![Image Size][shield-latest] | 稳定的最新标记镜像。 |
