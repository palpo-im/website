...
> ## Docker 用户 ⚠️
>
> 许多支持请求实际上是由 Docker 网络或 DNS 配置错误引起的，而非 Palpo 本身的问题。如果您正在使用 Docker，请在报告问题之前验证您的 Docker 设置（网络、DNS 解析、端口映射）。
>
...
Matrix 联邦非常繁重，会发送大量的 DNS 请求。不幸的是，这是设计使然，并且随着更多的服务器/目标解析步骤而变得更糟。Matrix 联邦也期望一个非常可靠的 DNS 设置。
